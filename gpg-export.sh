...
KEYPAIR_UID=$( gpg --options gpg-options.conf --list-keys | grep -e "^uid" | sed "${KEY_NUMBER?}q;d" | sed "s/^[^]]*][[:space:]]*//" | sed "s/^uid[[:space:]]*//" )
echo "Selected UID: \"${KEYPAIR_UID?}\""
KEYPAIR_EMAIL=$( echo "${KEYPAIR_UID?}" | sed -e "s/^[^<]*<//" | sed -e "s/>//" )
CMD="gpg --options gpg-options.conf --output \"${KEYPAIR_EMAIL?}.public\" --export \"${KEYPAIR_UID?}\""
echo $CMD
eval $CMD
